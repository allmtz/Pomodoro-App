...
export const Nav = ( { mode, manuallyChangeMode, settings }:INavProps ) => {
    ...
    const focusedStyling = ` p-4 rounded-full text-dark-bg text-center cursor-pointer duration-700`
    ...
    return(
        <nav className='flex max-sm:text-xs'>
            <ul className='flex items-center justify-around px-4 py-2 gap-9 text-light-purple font-bold bg-dark-bg rounded-full m-auto xl:scale-110'>
                <li className={mode === "pomoLength" ? `bg-${settings.color}` + focusedStyling : defaultStyling}
                onClick={()=>manuallyChangeMode("pomoLength")}
                >pomodoro</li>
                <li className={mode === "shortBreak" ? `bg-${settings.color}` + focusedStyling + focusedStyling : defaultStyling}
                onClick={()=>manuallyChangeMode("shortBreak")}
                >short break</li>
                <li className={mode === "longBreak" ? `bg-${settings.color}` + focusedStyling  + focusedStyling : defaultStyling}
